...
def main(cfg):
    ...
    DEVICE = torch.device("cuda" if cfg.hyperparameters.cuda else "cpu")
    # Data loading
    ...
    for epoch in range(cfg.hyperparameters.epochs):
        ...
        for batch_idx, (x, _) in enumerate(train_loader):
            ...
            loss.backward()
            optimizer.step()
        log.info("\tEpoch {epoch} + 1, complete!, \tAverage Loss: {loss}".format(epoch=epoch,loss=overall_loss / (batch_idx*cfg.hyperparameters.batch_size)))    
    log.info("Finish!!")
